...
<section class="site_container">
    <div class="page_heading">
        <h2 class="page_title">
            <?php if ( is_day() ) :
                $ale_title_for_heading = sprintf( esc_html__( 'Daily Archive: %s', 'gardener' ), get_the_date() );
            elseif ( is_month() ) :
                $ale_title_for_heading = sprintf( esc_html__( 'Monthly Archive: %s', 'gardener' ), get_the_date( _x( 'F Y', 'monthly archives date format', 'gardener' ) ) );
            elseif ( is_year() ) :
                $ale_title_for_heading = sprintf( esc_html__( 'Yearly Archive: %s', 'gardener' ), get_the_date( _x( 'Y', 'yearly archives date format', 'gardener' ) ) );
            else :
                $ale_title_for_heading = esc_html__( 'Archive', 'gardener' );
            endif;

            echo esc_attr($ale_title_for_heading); ?>
        </h2>
    </div>
    <div class="blog_cats">
        <div class="wrapper">
